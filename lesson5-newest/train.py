...
_ITERATION = 5100   #设置循环次数，每个为1个mini-batch，调试期间可以调小，训练再调整回来。
_IMAGE_SIZE = 0    
_LR = 0.1          # 学习率为0.1
_SEED = 220        # 随机种子
...
def Ttest(testImage,testL,seed):
    ...
    sumYesNum = _BATCH_SIZE * np.sum(train_accs)  #请补全  计算分类正确的图像张数 
...
optimizer=tf.train.AdamOptimizer(learning_rate).minimize(loss,global_step=global_step) # {0}   #请补全 构造优化器，采用Adam优化方法进行模型参数优化
...
for i in range(_ITERATION):
    ...
    i_global, _ = sess.run(optimizer,feed_dict={x: batch_xs, y: batch_ys})  #请补全 调用优化器进行当前mini-batch的训练优化，并返回global_step
...
sess.close() # 关闭会话
print("{:d} step cost time :{:f} s".format(int(_ITERATION), time() - total_timeS)) # 输出_ITERATION次迭代，总耗费时长，单位为s
