...
def plotData(X, y):
    ...
    p2 = plt.plot(X[neg, 0], X[neg, 1], marker='o', markersize=7, color='green')[0]   # david mark 2019.2.23
...
def BGD(X, y, iter_num, alpha):
    ...
    for i in range(iter_num):                       # 通过 for 循环结构，开始进行迭代，其中，i 可取的数依次为：0，1 ，2，....，iter_num-1， 迭代次数总共有 iter_num 次
        ...
        w -= (1.0/m)*alpha*trainMat.T*error     # 更新权重 w , BGD 批量梯度下降法 的核心， w = w - (1.0/m)*alpha*trainMat.T*error
    return w                                        # 返回 w
